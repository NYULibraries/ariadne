...
test('renders the Corriere Fiorentino against golden correctly', async ({ page }) => {
  ...
  fs.writeFileSync('e2e/actual/corriere-fiorentino.html', snapshot);
  const golden = fs.readFileSync('e2e/golden/corriere-fiorentino.html', 'utf8');
  ...
  if (stringifiedGolden !== stringifiedSnapshot) {
    try {
      execSync('diff -c e2e/golden/corriere-fiorentino.html e2e/actual/corriere-fiorentino.html');
    } catch (error) {
...
test.skip('renders the New Yorker against golden correctly', async ({ page }) => {
  ...
  const golden = fs.readFileSync('e2e/golden/new-yorker.html', { encoding: 'utf8' });
  const stringifiedSnapshot = JSON.stringify(snapshot);
  const stringifiedGolden = JSON.stringify(golden);
  const ok = stringifiedSnapshot === stringifiedGolden;
  ...
  if (stringifiedGolden !== stringifiedSnapshot) {
    try {
      ...
      execSync('diff -c e2e/golden/new-yorker.html e2e/actual/new-yorker.html');
    } catch (error) {
      // eslint-disable-next-line no-console
      console.log(error.stdout.toString());
      // eslint-disable-next-line no-console
