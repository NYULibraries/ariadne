...
const List = () => {
  ...
  const { elements, getElements, lastElement, error } = useApi(baseURl, query);
  ...
  return (
    <>
        ...
        {lastElement && (
          <div className="ask-librarian">
            <h6>
              <a
                style={{ textDecoration: 'none', color: '#6c07ae' }}
                href={lastElement.target_url}
                target="_blank"
                rel="noopener noreferrer"
              >
                {lastElement.target_public_name}
              </a>
            </h6>
          </div>
        )}
      ...
      {error && <div className="i-am-centered">{error}</div>}
    </>
  );
};
